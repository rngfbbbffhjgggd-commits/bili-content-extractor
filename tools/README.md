...
- **中文默认 0.6B**：中文视频（任意时长）一律用 Qwen3-ASR-0.6B（比 1.7B 快约 2 倍，自带标点）；非中文 <10 分钟用 Qwen3-ASR-1.7B（最准，52 语言），英/日/粤 ≥10 分钟用 Fun-ASR-Nano
- **英文引擎 Fun-ASR-Nano-2512**：英/日/粤长视频快档（标点+热词+中英混说）；非中文短视频仍归 1.7B（准确率更高）
...
python download_qwen06b.py    # 中文默认档 Qwen3-ASR-0.6B（处理中文视频必下）
python download_qwen17b.py    # 非中文短视频档 Qwen3-ASR-1.7B（最准）
...
### 非中文短视频（最准）：Qwen3-ASR-1.7B（int4 ONNX，3.9GB）
...
> 非中文且 <10 分钟自动用它（最准，52 语言）；也可 `--engine=qwen` 强制中文用它。

### 中文（默认，快档）：Qwen3-ASR-0.6B（int4 ONNX，约 2GB）
...
> 中文视频（任意时长）默认用它（同引擎体系，比 1.7B 快约 2 倍）。
...
> 英/日/粤 ≥10 分钟自动用它（标点+热词+中英混说；非中文短视频归 1.7B）。
...
python bili_quick.py <链接> --engine=qwen                            # 中文强制 Qwen3-ASR-1.7B（最准，较慢）
python bili_quick.py <链接> --engine=qwen06                          # 中文强制 Qwen3-ASR-0.6B（中文默认档）
