...
流程:
  ...
  无字幕: 下载音频 -> SenseVoice 内容语言检测 ->
          ...
          非中文 ≥10分钟    → Fun-ASR-Nano fp16（英/日/粤长视频，快档）
...
def zh_transcribe(audio_path, outdir, engine="auto", duration=0):
    ...
    if engine == "funasr":
        try:
            ...
            print(f"[!] funasr-nano 不可用 ({e})，回退 Qwen3-ASR-0.6B（中文默认）")
            engine = "qwen06"
    if engine == "sensevoice":
        try:
            ...
            print("[!] sensevoice_engine 不可用，回退 Qwen3-ASR-0.6B（中文默认）")
        except FileNotFoundError as e:
            print(f"[!] {e}，回退 Qwen3-ASR-0.6B（中文默认）")
        engine = "qwen06"
    if engine == "qwen06":
        print(f"[√] 加载 Qwen3-ASR-0.6B (中文默认,快档)...")
...
def transcribe_with_engine(wav, outdir, lang, duration, engine):
    """引擎分发（中文默认 0.6B 方案）:
       中文（任意时长）  → Qwen3-ASR-0.6B（中文默认，快档）
       非中文 <10分钟    → Qwen3-ASR-1.7B（最准，52语言）
       非中文 ≥10分钟    → Fun-ASR-Nano fp16（英/日/粤，标点+热词）
       Parakeet 已移除（无标点）
    ...
    if engine == "auto":
        if lang == "zh":
            ...
            engine = "qwen"
            print("[√] 自动选择引擎: Qwen3-ASR-1.7B (非中文短视频,最准)")
        else:
            engine = "funasr"
...
def ask_engine(engine, duration):
    ...
    print("  1) auto（推荐，直接回车）— 中文一律 Qwen3-ASR-0.6B；非中文<10分钟→1.7B；英日粤≥10分钟→Fun-ASR-Nano")
    print("  2) Qwen3-ASR-1.7B — 最准，多语言52种；较慢")
    print("  3) Qwen3-ASR-0.6B — 中文默认快档（比1.7B快约2倍）")
