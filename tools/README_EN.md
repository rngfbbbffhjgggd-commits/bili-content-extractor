...
Bilibili / YouTube video URL
   ...
   └─ No subtitles ──→ SenseVoice content language detection + local transcription
                       ├─ zh (any length)     → Qwen3-ASR-0.6B (Chinese default, fast tier)
                       ├─ non-zh <10min       → Qwen3-ASR-1.7B (most accurate)
                       ├─ en/ja/yue ≥10min    → Fun-ASR-Nano fp16 (fast tier)
...
- **Chinese default 0.6B**: Chinese videos (any length) use Qwen3-ASR-0.6B (~2× faster than 1.7B, punctuation included); non-Chinese <10min uses Qwen3-ASR-1.7B (most accurate, 52 languages), en/ja/yue ≥10min uses Fun-ASR-Nano
- **English engine Fun-ASR-Nano-2512**: fast tier for long en/ja/yue videos (punctuation + hotwords + code-switching); non-Chinese short videos stay on 1.7B (higher accuracy)
...
python download_qwen06b.py       # Chinese default tier Qwen3-ASR-0.6B (required for Chinese)
python download_qwen17b.py       # non-Chinese short-video tier Qwen3-ASR-1.7B (most accurate)
...
### Non-Chinese short videos (most accurate): Qwen3-ASR-1.7B (int4 ONNX, 3.9GB)
...
> Used automatically for non-Chinese videos <10 min (most accurate, 52 languages); `--engine=qwen` forces it for Chinese too.

### Chinese (default, fast tier): Qwen3-ASR-0.6B (int4 ONNX, ~2GB)
...
> Used by default for Chinese videos of any length (~2× faster than 1.7B, same engine family).
...
> Used automatically for en/ja/yue videos ≥10 min (punctuation + hotwords + code-switching; non-Chinese short videos stay on 1.7B).
...
python bili_quick.py <url> --engine=qwen                            # force Qwen3-ASR-1.7B (most accurate, slower)
python bili_quick.py <url> --engine=qwen06                          # force Qwen3-ASR-0.6B (Chinese default tier)
