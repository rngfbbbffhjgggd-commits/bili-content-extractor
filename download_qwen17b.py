#!/usr/bin/env python3
"""download_qwen17b.py — 下载 Qwen3-ASR-1.7B int4 ONNX（走 hf-mirror + 分片并行）

来源: andrewleech/qwen3-asr-1.7b-onnx
目标: D:\\BiliModels\\qwen3-asr-1.7b\\qwen3-asr-1.7b-int4\\
用法: python download_qwen17b.py [并发数]
"""
import os
import subprocess
import sys
...
BASE = "https://hf-mirror.com/andrewleech/qwen3-asr-1.7b-onnx/resolve/main"
DEST_ROOT = r"D:\BiliModels\qwen3-asr-1.7b\qwen3-asr-1.7b-int4"
...
def main():
    ...
    for rel in FILES:
        ...
        if got < size:
            ...
            sys.exit(1)
    print("\n[√] Qwen3-ASR-1.7B int4 全部就绪:", DEST_ROOT, flush=True)
